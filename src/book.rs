...
pub fn polyglot_key(board: &Board) -> u64 {
    ...
    for sq in *board.combined() {
        ...
        let kind = piece_kind_index(piece, color);
        let idx = RANDOM_PIECE + 64 * kind + sq.to_index();
        key ^= RANDOM64[idx];
    }
    ...
    if let Some(ep_sq) = board.en_passant() {
        key ^= RANDOM64[RANDOM_EN_PASSANT + ep_sq.get_file().to_index()];
    }
...
impl OpeningBook {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        if bytes.len() % 16 != 0 || bytes.is_empty() {
            return None;
        }
    ...
    pub fn probe(&mut self, board: &Board) -> Option<ChessMove> {
        ...
        let candidates: Vec<BookEntry> = self.entries[start..end].to_vec();
        ...
        let mut pick = self.next_rand() % total_weight;

        for entry in &candidates {
            let w = entry.weight.max(1) as u64;
            if pick < w {
                if let Some(m) = decode_move(board, entry.mv) {
                    return Some(m);
                }
                return None;
            }
...
fn decode_move(board: &Board, bits: u16) -> Option<ChessMove> {
    if bits == 0 {
        return None;
    }
    ...
    let promo_bits = (bits >> 12) & 0x7;

    let from_sq = Square::make_square(Rank::from_index(from_row), File::from_index(from_file));
    let mut to_sq = Square::make_square(Rank::from_index(to_row), File::from_index(to_file));
    ...
    if board.piece_on(from_sq) == Some(Piece::King) {
        let (home_rank, kingside_rook, queenside_rook) = match board.color_on(from_sq) {
            Some(Color::White) => (Rank::First, Square::H1, Square::A1),
            Some(Color::Black) => (Rank::Eighth, Square::H8, Square::A8),
            None => return None,
        };

        if from_sq.get_rank() == home_rank {
            if to_sq == kingside_rook {
                to_sq = Square::make_square(home_rank, File::G);
            } else if to_sq == queenside_rook {
                to_sq = Square::make_square(home_rank, File::C);
            }
        }
    }
