...
#[inline]
fn pst_index(color: Color, file: usize, rank: usize) -> usize {
    match color {
...
pub(crate) fn pst_score(board: &Board) -> (i32, i32, i32) {
    ...
    for sq in *board.combined() {
        ...
        let idx = pst_index(color, file, rank);

        let (mg, eg, weight) = match piece {
            Piece::Pawn => (PAWN_MG[idx], PAWN_EG[idx], 0),
            Piece::Knight => (KNIGHT_MG[idx], KNIGHT_EG[idx], PHASE_WEIGHT_KNIGHT),
            Piece::Bishop => (BISHOP_MG[idx], BISHOP_EG[idx], PHASE_WEIGHT_BISHOP),
            Piece::Rook => (ROOK_MG[idx], ROOK_EG[idx], PHASE_WEIGHT_ROOK),
            Piece::Queen => (QUEEN_MG[idx], QUEEN_EG[idx], PHASE_WEIGHT_QUEEN),
            Piece::King => (KING_MG[idx], KING_EG[idx], 0),
        };
